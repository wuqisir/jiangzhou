...
class NGCF(nn.Module):
    def __init__(self, n_user, n_item, norm_adj, args):
        ...
        self.train_matrix = np.load(f"./dataset/{args.dataset}/{args.dataset}_train.npy")
    ...
    def create_bpr_loss(self, users, pos_items, neg_items, drop_flag=False):
        ...
        # true_pos_score = torch.zeros_like(torch.tensor(pos_items))
        # true_neg_score = torch.zeros_like(torch.tensor(neg_items))
        # for i in range(len(users)):
        #     true_pos_score[i] = self.train_matrix[users[i], pos_items[i]]
        #     true_neg_score[i] = self.train_matrix[users[i], neg_items[i]]
        #
        # true_neg_score = true_neg_score.to(torch.float32)
        # true_pos_score = true_pos_score.to(torch.float32)
        maxi = nn.LogSigmoid()(pos_scores - neg_scores)
        # Loss = nn.MSELoss()
        # loss = Loss(pos_scores, true_pos_score.to(args.device)) + Loss(neg_scores, true_neg_score.to(args.device))
        # loss = Loss(pos_scores, true_pos_score) + Loss(neg_scores, true_neg_score)
    ...
    def forward(self, users, pos_items, drop_flag=False):
        u_g_embeddings, pos_i_g_embeddings, _ = self.get_embedding(users, pos_items,[], drop_flag)
